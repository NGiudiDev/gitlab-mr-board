...
- **El layout y la navegación viven en `frontend/src/app/`**: `AppShell.jsx` contiene la barra superior y el único `main`; `routes.js`, las rutas y secciones navegables; y `AccountMenu.jsx`, el avatar y su desplegable para abrir «Mi perfil» o «Mi cuenta» y cerrar la sesión. `App.jsx` declara las pantallas con React Router. Al agregar una sección, sumar su URL a `APP_PATHS`, su navegación a `NAVIGATION_SECTIONS`, su `Route` a `App.jsx` y actualizar [`docs/architecture/frontend.md`](docs/architecture/frontend.md#navegación-entre-secciones).
