...
function createAccountService(options) {
   ...
   */
  async function toSummary(account, includeInviteCode) {
    return {
      id: account.id,
      name: account.name,
  ...
  async function getSummary(accountId, includeInviteCode) {
  ...
  async function rename(accountId, name) {
  ...
  async function rotateInviteCode(accountId) {
    const account = await requireAccount(accountId);
