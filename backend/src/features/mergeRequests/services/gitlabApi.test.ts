...
import type { GitLabTestItem } from '../../../../test/types.js';
...
function stubFetch(handler: (url: string) => Response | Promise<Response> = () => jsonResponse({})) {
  ...
  return {
    ...
    urlOf: (call = 0) => new URL(fetchMock.mock.calls[call]?.[0] as string),
  };
}
