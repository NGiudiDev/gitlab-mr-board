...
const resolvedThreads = { status: 'resolved', unresolvedCount: 0 };
const openThreads = { status: 'open', unresolvedCount: 2 };
const successPipeline = { status: 'success', pipelineUrl: 'https://gitlab.example.com/pipe/1' };
...
describe('computeMergeability', () => {
  ...
  it('no bloquea cuando el estado de las aprobaciones es desconocido', () => {
    const unknownApprovals = { status: 'unknown', required: 0, given: 0 };
    const mr = buildMergeRequest({ labels: ['qa_approved'] });
...
describe('computeResponsiblePeople', () => {
  ...
  it('mantiene sólo los reviewers que todavía no aprobaron', () => {
    ...
    const approvals = { ...pendingApprovals, approvers: ['BETO'] };
  ...
  it('devuelve la responsabilidad al autor cuando todos los reviewers aprobaron', () => {
    const mr = buildMergeRequest({ reviewers: [beto, caro] });
    const approvals = { ...pendingApprovals, approvers: ['beto', 'caro'] };
...
describe('collectPeople', () => {
  function participants(author, authorUsername, reviewers = []) {
    return {
      author,
      authorUsername,
