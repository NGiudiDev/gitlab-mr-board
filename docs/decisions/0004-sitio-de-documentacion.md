...
La documentación gana búsqueda y navegación por secciones, y el build actúa como prueba de integridad de los enlaces internos. El costo es una dependencia de desarrollo con su árbol transitivo y la obligación de sumar cada documento nuevo al sidebar —si se olvida, la página existe pero queda fuera de la navegación—. Como no hay publicación automática, el sitio sólo existe para quien lo levanta: el repositorio sigue siendo la fuente de lectura por defecto. Si más adelante se publica, todo lo que esté en `docs/` pasa a ser público.
